...
5. 复杂类、结构体、公共方法使用 Doxygen 风格注释。
6. 复杂方法中的关键步骤必须用 `//` 标明原理、用途或约束，尤其是 OpenGL 生命周期、线程同步、GPU/CPU 数据同步、ROAM 拓扑提交和跨平台分支。
7. 私有短方法一般不需要注释；如果逻辑复杂，应加简短说明或拆分。
8. 注释必须随代码更新，不能留下过期解释。
9. 项目自有 C++ 源码的注释覆盖率应大于等于 15%，按“注释行数 / 非空源码行数”粗略统计；不要用无意义注释凑覆盖率。
