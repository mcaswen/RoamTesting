...
5. 复杂类、结构体、枚举和公共方法使用 Doxygen 风格注释。
6. `struct` 不能只给字段写注释，结构体本身也要说明用途、所属阶段或数据流位置。
7. 内部临时 helper 只在语义不明显时补注释，避免为简单聚合类型制造噪音。
8. 复杂方法中的关键步骤必须用 `//` 标明原理、用途或约束，尤其是 OpenGL 生命周期、线程同步、GPU/CPU 数据同步、ROAM 拓扑提交和跨平台分支。
9. 私有短方法一般不需要注释；如果逻辑复杂，应加简短说明或拆分。
10. 注释必须随代码更新，不能留下过期解释。
11. 项目自有 C++ 源码的注释覆盖率应大于等于 15%，按“注释行数 / 非空源码行数”粗略统计；不要用无意义注释凑覆盖率。
...
```cpp
/// @brief 更新当前帧正在使用的地形 LOD 算法
/// @param frame 共享的时间和帧序号上下文
/// @param camera 用于视点相关 LOD 判断的相机数据
void Update(const FrameContext& frame, const CameraData& camera);
```
