...
8. 公共方法只有在参数约束、调用顺序、生命周期、失败语义或跨模块边界不显然时才写方法注释。
9. 内部临时 helper 只在语义不明显时补注释，避免为简单聚合类型制造噪音。
10. 复杂方法中的关键步骤必须用 `//` 标明原理、用途或约束，尤其是 OpenGL 生命周期、线程同步、GPU/CPU 数据同步、ROAM 拓扑提交和跨平台分支。
11. 私有短方法一般不需要注释；如果逻辑复杂，应加简短说明或拆分。
12. 注释必须随代码更新，不能留下过期解释。
