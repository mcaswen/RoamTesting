...
- 详细 CSV：`runtime-benchmark-20260828-200404.csv`
...
- Benchmark 标签：five-round-1280x720-default-r05
...
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| Classic CPU ROAM | 600 | 5.51 | 12.27 | 5.27 | 11.78 | 18758.02 | 19608 | 117377.00 | 205068 | 97.11 | 443.74 | 1 | 20 | 19 | 0 |
| Data-Oriented CPU ROAM | 600 | 4.05 | 9.71 | 3.81 | 9.49 | 18758.02 | 19608 | 117377.00 | 205068 | 203.21 | 4210.62 | 8 | 20 | 19 | 0 |
| CBT 2024 | 600 | 0.33 | 0.88 | 0.05 | 0.15 | 18732.14 | 20028 | 18732.14 | 20028 | 0.00 | 0.00 | 0 | 20 | 20 | 0 |
...
- dropped/错代样本：0
- 最大诊断样本年龄：2 帧
- 平均 GPU 阶段合计：0.0791 ms
- 最大 GPU 阶段合计：0.2922 ms
...
| Bisect | 0.0043 |
| PropagateBisect | 0.0043 |
| PrepareSimplify | 0.0042 |
| Simplify | 0.0051 |
| PropagateSimplify | 0.0042 |
| ReducePre | 0.0014 |
| ReduceFirst | 0.0022 |
| ReduceSecond | 0.0011 |
| Indexation / indirect | 0.0049 |
| Render vertex evaluation | 0.0052 |
| Validation | 0.0015 |
...
| --- | ---: | ---: | --- |
| Prepare / 帧状态 | 0.1863 | 0.2658 | 准备持久拓扑和逐帧输入 |
| Merge 候选评分 | 0.6809 | 0.2567 | 刷新持久 Q_m 的优先级 |
| Merge 拓扑提交 / 向上级联 | 0.1089 | 0.1967 | 提交 diamond merge、邻接修复和级联回收 |
| Split 前 active leaf 收集 | 0.0000 | 0.0000 | 两者均直接复用持久活动叶表示 |
| 视点相关 leaf error / 视锥测试 | 0.0000 | 0.0000 | 计入持久 Q_s 优先级刷新，因此独立字段为 0 |
| Split 扫描/标记 | 2.2046 | 0.6175 | 刷新 Q_s、建堆并生成提交候选 |
| Split 拓扑 / 裂缝约束提交 | 0.2341 | 0.3536 | 按同一预算与 forced-split 约束提交 |
| 细分后 active leaf 收集 / 输出视图 | 0.0000 | 0.0000 | Classic 复用 dense slot owners，DOD 复用 ActiveLeafNodes |
| Mesh emit / draw argument 生成 | 0.2221 | 0.2704 | 两者均只重写 dirty slot；DOD 可将较大批次分段给 worker |
| Finalize / 发布 packet | 1.4523 | 1.6806 | 汇总统计、更新跨帧状态并发布 renderer packet |
...
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| Classic CPU ROAM | 5.0895 | 0.1863 | 0.6809 | 0.1089 | 0.0000 | 0.0000 | 2.2046 | 0.2341 | 0.0000 | 0.2221 | 1.4523 | 0.1731 |
| Data-Oriented CPU ROAM | 3.6415 | 0.2658 | 0.2567 | 0.1967 | 0.0000 | 0.0000 | 0.6175 | 0.3536 | 0.0000 | 0.2704 | 1.6806 | 0.1586 |
...
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| Classic Split | 0 | 0 | 0 | 0 | 0 | 0.2341 | 0.2341 | 0.2341 |
| DoD Split | 0.0000 | 0.0000 | 0.0000 | 0.0000 | 0.0000 | 0.3534 | 0.3534 | 0.3536 |
| Merge | 0.0108 | 0.0000 | 0.0000 | 0.0000 | 0.0000 | 0.1834 | 0.1942 | 0.1967 |
...
| --- | ---: | ---: | ---: | ---: |
| Classic CPU ROAM | 2.4387 | 0.7898 | 0.2221 | 0.0000 |
| Data-Oriented CPU ROAM | 0.9711 | 0.4534 | 0.2704 | 0.0000 |
| CBT 2024 | 0.0267 | 0.0135 | 0.0055 | 0.0015 |

## 渲染与上传

| Algorithm | Frame fence wait | Render | Max render | Max upload B | Max readback B |
| --- | ---: | ---: | ---: | ---: | ---: |
| Classic CPU ROAM | 0.00 | 0.01 | 0.02 | 3294144 | 0 |
| Data-Oriented CPU ROAM | 0.00 | 0.01 | 0.02 | 516936 | 0 |
| CBT 2024 | 0.10 | 0.09 | 0.31 | 0 | 428 |
