...
- 详细 CSV：`runtime-benchmark-20260817-225602.csv`

- 构建配置：RelWithDebInfo
- 图形后端：OpenGL
- 图形适配器：NVIDIA GeForce RTX 5090 D/PCIe/SSE2 (4.3.0 NVIDIA 591.86)
- Benchmark 标签：C1-default
...
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| Classic CPU ROAM | 600 | 1.81 | 29.00 | 1.62 | 26.34 | 10699.66 | 12629 | 37578.82 | 49326 | 97.44 | 2853.62 | 1 | 14 | 14 | 0 |
| Data-Oriented CPU ROAM | 600 | 2.47 | 94.04 | 1.95 | 17.62 | 10699.66 | 12629 | 37578.82 | 49326 | 298.16 | 11563.61 | 8 | 14 | 14 | 0 |
| GPU ROAM-like | 600 | 4.09 | 22.05 | 3.77 | 21.61 | 10699.66 | 12629 | 37578.82 | 49326 | 149.68 | 4509.01 | 8 | 14 | 14 | 0 |
...
| --- | ---: | ---: | ---: | ---: | --- |
| Prepare / 帧状态 | 0.0852 | 0.0951 | 0.1426 | N/A | CPU 准备持久拓扑和 snapshot 输入 |
| Merge 候选评分 | 0.4048 | 0.1195 | 0.2026 | 0.0050 | Shader 为诊断重新扫描已 split 的父节点；GPU 不提交这些候选 |
| Merge 拓扑提交 / 向上级联 | 0.0255 | 0.0425 | 0.0519 | N/A | 持久 merge、邻接修复和级联回收由 Classic/DOD CPU 路径执行；GPU shader 未提交 merge |
| Split 前 active leaf 收集 | 0.0000 | 0.0000 | 0.0000 | 0.0041 | DOD CPU 直接以持久 Q_s 表示 active leaf 与预算计数，本列为 0；GPU 在评分前仍单独压缩 snapshot |
| 视点相关 leaf error / 视锥测试 | 0.0000 | 0.0000 | 0.0000 | 0.0046 | Classic 与 DOD 都把评分计入持久 Q_s 优先级刷新，本列为 0；GPU 基于 snapshot 单独评分 |
| Split 扫描/标记 | 0.4362 | 0.1946 | 0.4349 | 0.0037 | DOD CPU 数值包含持久 Q_s 的并行 SSE/视锥优先级刷新、原地建堆和提交快照生成；GPU append 顺序不是误差优先级顺序 |
| Split 拓扑 / 裂缝约束提交 | 0.0659 | 0.0712 | 0.0825 | 0.0031 | GPU 只提交一轮受预算约束的细分，并只处理直接 base-neighbor diamond pair；没有递归 forced-split 链 |
| 细分后 active leaf 收集 / 输出视图 | 0.0000 | 0.0000 | 0.0000 | 0.0066 | Classic 直接复用 dense mesh slot owners，DOD 直接复用 ActiveLeafNodes，因此两者本列均为 0；GPU 数值包含 counter reset 和 split 后 leaf compaction |
| Mesh emit / draw argument 生成 | 0.0373 | 0.6675 | 0.0000 | 0.0099 | Classic 只重写拓扑变化影响的稳定槽位；DOD 仍完整 emit；GPU 输出非共享顶点、索引和 indirect draw argument |
| Finalize / 发布 packet | 0.5376 | 0.5013 | 0.6937 | N/A | CPU 发布统计和 renderer 资源契约 |
...
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| Classic CPU ROAM | 1.5928 | 0.0852 | 0.4048 | 0.0255 | 0.0000 | 0.0000 | 0.4362 | 0.0659 | 0.0000 | 0.0373 | 0.5376 | 0.0239 |
| Data-Oriented CPU ROAM | 1.6919 | 0.0951 | 0.1195 | 0.0425 | 0.0000 | 0.0000 | 0.1946 | 0.0712 | 0.0000 | 0.6675 | 0.5013 | 0.2155 |
| GPU ROAM-like | 1.6085 | 0.1426 | 0.2026 | 0.0519 | 0.0000 | 0.0000 | 0.4349 | 0.0825 | 0.0000 | 0.0000 | 0.6937 | 0.5464 |
...
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| Classic Split | 0 | 0 | 0 | 0 | 0 | 0.0659 | 0.0659 | 0.0659 |
| DoD Split | 0.0000 | 0.0000 | 0.0000 | 0.0000 | 0.0000 | 0.0710 | 0.0710 | 0.0712 |
| Merge | 0.0027 | 0.0000 | 0.0000 | 0.0000 | 0.0000 | 0.0384 | 0.0411 | 0.0425 |
...
| --- | ---: | ---: | ---: | ---: |
| Classic CPU ROAM | 0.5021 | 0.4303 | 0.0373 | 0.0000 |
| Data-Oriented CPU ROAM | 0.2658 | 0.1620 | 0.6675 | 0.0000 |
| GPU ROAM-like | 0.5174 | 0.2545 | 0.0000 | 0.0000 |
...
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| GPU ROAM-like | 0.0041 | 0.0046 | 0.0037 | 0.0050 | 0.0031 | 0.0028 | 0.0038 | 0.0099 | 0.0369 | 0.0638 |
...
| GPU ROAM-like | 1.00 | 0.10 | 0.04 | 0.00 | 0.35 | 0.00 | 0.00 | 0.00 | 5524564 | 96 |
