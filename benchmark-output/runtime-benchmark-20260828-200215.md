...
- 详细 CSV：`runtime-benchmark-20260828-200215.csv`
...
- Benchmark 标签：five-round-1280x720-default-r03
...
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| Classic CPU ROAM | 600 | 4.41 | 11.19 | 4.23 | 10.81 | 18758.02 | 19608 | 117377.00 | 205068 | 99.63 | 468.91 | 1 | 20 | 19 | 0 |
| Data-Oriented CPU ROAM | 600 | 3.79 | 9.35 | 3.58 | 8.87 | 18758.02 | 19608 | 117377.00 | 205068 | 240.78 | 5041.95 | 8 | 20 | 19 | 0 |
| CBT 2024 | 600 | 0.33 | 0.82 | 0.04 | 0.17 | 18732.14 | 20028 | 18732.14 | 20028 | 0.00 | 0.00 | 0 | 20 | 20 | 0 |
...
- dropped/错代样本：0
- 最大诊断样本年龄：2 帧
- 平均 GPU 阶段合计：0.0787 ms
- 最大 GPU 阶段合计：0.3006 ms
...
| --- | ---: | ---: | --- |
| Prepare / 帧状态 | 0.1570 | 0.2463 | 准备持久拓扑和逐帧输入 |
| Merge 候选评分 | 0.5873 | 0.2538 | 刷新持久 Q_m 的优先级 |
| Merge 拓扑提交 / 向上级联 | 0.0894 | 0.1841 | 提交 diamond merge、邻接修复和级联回收 |
| Split 前 active leaf 收集 | 0.0000 | 0.0000 | 两者均直接复用持久活动叶表示 |
| 视点相关 leaf error / 视锥测试 | 0.0000 | 0.0000 | 计入持久 Q_s 优先级刷新，因此独立字段为 0 |
| Split 扫描/标记 | 1.8318 | 0.6372 | 刷新 Q_s、建堆并生成提交候选 |
| Split 拓扑 / 裂缝约束提交 | 0.2003 | 0.3335 | 按同一预算与 forced-split 约束提交 |
| 细分后 active leaf 收集 / 输出视图 | 0.0000 | 0.0000 | Classic 复用 dense slot owners，DOD 复用 ActiveLeafNodes |
| Mesh emit / draw argument 生成 | 0.1778 | 0.2552 | 两者均只重写 dirty slot；DOD 可将较大批次分段给 worker |
| Finalize / 发布 packet | 1.0183 | 1.5034 | 汇总统计、更新跨帧状态并发布 renderer packet |
...
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| Classic CPU ROAM | 4.0621 | 0.1570 | 0.5873 | 0.0894 | 0.0000 | 0.0000 | 1.8318 | 0.2003 | 0.0000 | 0.1778 | 1.0183 | 0.1609 |
| Data-Oriented CPU ROAM | 3.4137 | 0.2463 | 0.2538 | 0.1841 | 0.0000 | 0.0000 | 0.6372 | 0.3335 | 0.0000 | 0.2552 | 1.5034 | 0.1532 |
...
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| Classic Split | 0 | 0 | 0 | 0 | 0 | 0.2003 | 0.2003 | 0.2003 |
| DoD Split | 0.0000 | 0.0000 | 0.0000 | 0.0000 | 0.0000 | 0.3332 | 0.3332 | 0.3335 |
| Merge | 0.0091 | 0.0000 | 0.0000 | 0.0000 | 0.0000 | 0.1730 | 0.1820 | 0.1841 |
...
| --- | ---: | ---: | ---: | ---: |
| Classic CPU ROAM | 2.0321 | 0.6767 | 0.1778 | 0.0000 |
| Data-Oriented CPU ROAM | 0.9706 | 0.4379 | 0.2552 | 0.0000 |
| CBT 2024 | 0.0275 | 0.0131 | 0.0053 | 0.0015 |

## 渲染与上传

| Algorithm | Frame fence wait | Render | Max render | Max upload B | Max readback B |
| --- | ---: | ---: | ---: | ---: | ---: |
| Classic CPU ROAM | 0.00 | 0.01 | 0.01 | 3294144 | 0 |
| Data-Oriented CPU ROAM | 0.00 | 0.01 | 0.20 | 516936 | 0 |
| CBT 2024 | 0.12 | 0.09 | 0.40 | 0 | 428 |
