...
- 相机路径：从地形边缘中点移动到地形中心
- 每种算法的采样时长：10 秒
- 详细 CSV：`runtime-benchmark-20260810-001900.csv`
...
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| Classic CPU ROAM | 5660 | 1.77 | 34.43 | 1.60 | 26.57 | 10542.64 | 12629 | 38473.74 | 49326 | 107.41 | 2824.99 | 1 | 14 | 14 | 0 |
| Data-Oriented CPU ROAM | 3022 | 3.32 | 96.79 | 2.88 | 32.02 | 10519.63 | 12629 | 38658.37 | 49326 | 273.18 | 7375.07 | 8 | 14 | 14 | 0 |
| GPU ROAM-like | 2424 | 4.14 | 36.33 | 3.88 | 35.93 | 10606.56 | 12629 | 38093.28 | 49326 | 142.10 | 3984.83 | 8 | 14 | 14 | 0 |
...
| --- | ---: | ---: | ---: | ---: | --- |
| Prepare / 帧状态 | 0.1149 | 0.1885 | 0.1777 | N/A | CPU 准备持久拓扑和 snapshot 输入 |
| Merge 候选评分 | 0.4338 | 0.1717 | 0.1825 | 0.0050 | Shader 为诊断重新扫描已 split 的父节点；GPU 不提交这些候选 |
| Merge 拓扑提交 / 向上级联 | 0.0042 | 0.0237 | 0.0253 | N/A | 持久 merge、邻接修复和级联回收由 Classic/DOD CPU 路径执行；GPU shader 未提交 merge |
| Split 前 active leaf 收集 | 0.0000 | 0.0000 | 0.0000 | 0.0041 | DOD CPU 直接以持久 Q_s 表示 active leaf 与预算计数，本列为 0；GPU 在评分前仍单独压缩 snapshot |
| 视点相关 leaf error / 视锥测试 | 0.0000 | 0.0000 | 0.0000 | 0.0071 | Classic 与 DOD 都把评分计入持久 Q_s 优先级刷新，本列为 0；GPU 基于 snapshot 单独评分 |
| Split 扫描/标记 | 0.4445 | 0.3240 | 0.4667 | 0.0036 | DOD CPU 数值包含持久 Q_s 的并行 SSE/视锥优先级刷新、原地建堆和提交快照生成；GPU append 顺序不是误差优先级顺序 |
| Split 拓扑 / 裂缝约束提交 | 0.0089 | 0.0361 | 0.0431 | 0.0030 | GPU 只提交一轮受预算约束的细分，并只处理直接 base-neighbor diamond pair；没有递归 forced-split 链 |
| 细分后 active leaf 收集 / 输出视图 | 0.0000 | 0.0000 | 0.0000 | 0.0065 | Classic 直接复用 dense mesh slot owners，DOD 直接复用 ActiveLeafNodes，因此两者本列均为 0；GPU 数值包含 counter reset 和 split 后 leaf compaction |
| Mesh emit / draw argument 生成 | 0.0048 | 0.9902 | 0.0000 | 0.0099 | Classic 只重写拓扑变化影响的稳定槽位；DOD 仍完整 emit；GPU 输出非共享顶点、索引和 indirect draw argument |
| Finalize / 发布 packet | 0.5768 | 0.8575 | 0.8378 | N/A | CPU 发布统计和 renderer 资源契约 |
...
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| Classic CPU ROAM | 1.5884 | 0.1149 | 0.4338 | 0.0042 | 0.0000 | 0.0000 | 0.4445 | 0.0089 | 0.0000 | 0.0048 | 0.5768 | 0.0039 |
| Data-Oriented CPU ROAM | 2.5920 | 0.1885 | 0.1717 | 0.0237 | 0.0000 | 0.0000 | 0.3240 | 0.0361 | 0.0000 | 0.9902 | 0.8575 | 0.2337 |
| GPU ROAM-like | 1.7334 | 0.1777 | 0.1825 | 0.0253 | 0.0000 | 0.0000 | 0.4667 | 0.0431 | 0.0000 | 0.0000 | 0.8378 | 0.4530 |
...
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| Classic Split | 0 | 0 | 0 | 0 | 0 | 0.0089 | 0.0089 | 0.0089 |
| DoD Split | 0.0000 | 0.0000 | 0.0000 | 0.0000 | 0.0000 | 0.0358 | 0.0358 | 0.0361 |
| Merge | 0.0020 | 0.0000 | 0.0000 | 0.0000 | 0.0000 | 0.0197 | 0.0218 | 0.0237 |
...
| --- | ---: | ---: | ---: | ---: | ---: |
| Classic CPU ROAM | 1 | 18.6617 | 10523.9804 | 10.1246 | 2121672 |
...
| --- | ---: | ---: | ---: | ---: |
| Classic CPU ROAM | 0.4534 | 0.4381 | 0.0048 | 0.0000 |
| Data-Oriented CPU ROAM | 0.3601 | 0.1955 | 0.9902 | 0.0000 |
| GPU ROAM-like | 0.5098 | 0.2078 | 0.0000 | 0.0000 |
...
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| GPU ROAM-like | 0.0041 | 0.0071 | 0.0036 | 0.0050 | 0.0030 | 0.0027 | 0.0038 | 0.0099 | 0.0392 | 0.2485 |
...
| GPU ROAM-like | 1.00 | 0.06 | 0.05 | 0.00 | 0.41 | 0.00 | 0.00 | 0.00 | 5524564 | 96 |
